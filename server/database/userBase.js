...
module.exports=
{
    Do_signup : (data)=>
    {
        return new promise(async(resolve,reject)=>
        {
            ...
            await schema.Schema_For_UserSignup().then((YourModel)=>
            {
                ...
                db.get().collection(consts.user_Base).insertOne(dataObject).then((data)=>
                {
                   // console.log(data);
                    resolve(data)
                })
    ...
    Get_A_pRoduct_single_details : (Id)=>
    {
        return new promise(async(resolve,reject)=>
        {
            await db.get().collection(consts.admin_Pro).findOne({_id:objectId(Id)}).then((pro)=>
            {
                resolve(pro)
            })
        })
    ...
    Do_Cart : (proid,userid)=>
    {
        return new promise(async(resolve,reject)=>
        {
            ...
            await db.get().collection(consts.cart_bade).findOne({userId:objectId(userid)}).then(async(pro)=>
            {
                if(pro)
                {
        ...
        if (st != -1) {
        //console.log("hi..")
        await db.get().collection(consts.cart_bade).updateOne({ "product.proId": objectId(proid), userId: objectId(userid) },
            {
                $inc: { "product.$.qut": 1 }
            }).then((data) => {
                resolve(data)
            ...
            await db.get().collection(consts.cart_bade).updateOne({ userId: objectId(userid) },
            {
                $push:
                {
                    product: state
                ...
                else
                {
                    ...
                    db.get().collection(consts.cart_bade).insertOne(details).then((data)=>
                    {
                        resolve(data)
                    })
                }
    ...
    View_Carted_products:(userid)=>
    {
        return new promise(async(resolve,reject)=>
        {
            var pro = await db.get().collection(consts.cart_bade).aggregate([
                {
                    $match:
                    {
                        userId:objectId(userid)
    ...
    Total_price_for_carted_products : (userid)=>
    {
            return new promise(async(resolve,reject)=>
            {
                var total = await db.get().collection(consts.cart_bade).aggregate([
                    {
                        $match:
                        {
                            userId:objectId(userid)
    ...
    Find_Whicch_User_Is_CaRTed : (userid)=>
    {
        return new promise(async(resolve,reject)=>
        {
            await db.get().collection(consts.user_Base).findOne({_id:objectId(userid)}).then((user)=>
            {
                resolve(user)
            })
        })
    ...
    Cart_products_for_order : (userid)=>
    {
        return new promise(async(resolve,reject)=>
        {
            await db.get().collection(consts.cart_bade).findOne({userId:objectId(userid)}).then((pro)=>
            {
                resolve(pro)
            })
        })
    ...
    Place_Order : (data)=>
    {
        ...
        {
            db.get().collection(consts.order_base).insertOne(data).then((res)=>
            {
                resolve(res)
            })
        })
    ...
    Remove_product_From_cart_after_place_Order : (userid)=>
    {
        return new promise(async(resolve,reject)=>
        {
            await db.get().collection(consts.cart_bade).deleteOne({userId:objectId(userid)}).then((data)=>
            {
                resolve(data)
            })
        })
